...
import { Calendar, DollarSign, Users, TrendingUp, AlertTriangle, Tv, Plus, Edit2, Trash2, Check, X, Send, MapPin, FileText, Phone, Filter, Repeat, Lock, Download, Cloud } from 'lucide-react';
...
const LionTechDashboard = () => {
  ...
  const ADMIN_PASSWORD = 'liontech2025';
  ...
  const executeDeletePoint = async () => {
    if (deletePassword !== ADMIN_PASSWORD) { alert('❌ Senha incorreta!'); return; }
  ...
  const availableMonths = useMemo(() => [...new Set(payments.map(p => p.month))].sort().reverse(), [payments]);
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            ...
            <input type="password" value={deletePassword} onChange={(e) => setDeletePassword(e.target.value)} placeholder="Senha admin" className="w-full border rounded-lg px-4 py-2 mb-4" />
            <p className="text-xs text-slate-500 mb-4">Senha: liontech2025</p>
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-700 text-white shadow-lg">
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={loadAllData} className="bg-blue-500 hover:bg-blue-600 px-3 py-2 rounded flex items-center gap-2">
              <Cloud size={16} />Recarregar
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-emerald-600 rounded-xl shadow-lg p-6 text-white">
              <div className="flex justify-between items-center">
                ...
                <button onClick={generateMonthlyCharges} className="bg-white text-emerald-600 px-6 py-3 rounded-lg font-semibold hover:bg-emerald-50"><Plus size={20} /> Gerar</button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {tvPoints.map(point => (
                <div key={point.id} className="bg-white rounded-xl shadow-md p-5 border-l-4 border-emerald-500">
                  ...
                  <div className="flex gap-2 mt-4">
                    <button onClick={() => { setEditingPoint(point.id); setPointForm(point); }} className="flex-1 bg-blue-50 text-blue-600 px-3 py-2 rounded-lg"><Edit2 size={14} /></button>
                    <button onClick={() => confirmDeletePoint(point.id)} className="flex-1 bg-red-50 text-red-600 px-3 py-2 rounded-lg"><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <input type="number" placeholder="Dia Vencimento *" value={clientForm.dueDay} onChange={(e) => setClientForm({...clientForm, dueDay: e.target.value})} className="border rounded-lg px-4 py-2" />
                    <input type="number" placeholder="Desconto (R$)" value={clientForm.discountValue} onChange={(e) => setClientForm({...clientForm, discountValue: e.target.value})} className="border rounded-lg px-4 py-2" />
                  </div>
                  <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {filteredClients.map(client => (
                <div key={client.id} className="bg-white rounded-xl shadow-md p-5 border-l-4 border-blue-500">
                  ...
                  <div className="flex gap-2 mt-4">
                    <button onClick={() => viewContract(client)} className="flex-1 bg-emerald-50 text-emerald-600 px-3 py-2 rounded-lg text-sm"><FileText size={14} /></button>
                    <button onClick={() => { setEditingClient(client.id); setClientForm(client); }} className="flex-1 bg-blue-50 text-blue-600 px-3 py-2 rounded-lg text-sm"><Edit2 size={14} /></button>
                    <button onClick={() => confirmDeleteClient(client.id)} className="flex-1 bg-red-50 text-red-600 px-3 py-2 rounded-lg text-sm"><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={generateMonthlyCharges} className="bg-emerald-600 text-white px-4 py-2 rounded-lg"><Plus size={20} /></button>
              </div>
            </div>
                  ...
                  {filteredPayments.map(payment => {
                    ...
                    return (
                      <tr key={payment.id} className="border-t">
                        ...
                        <td className="px-4 py-3"><span className={`px-3 py-1 rounded-full text-xs ${payment.status === 'Pago' ? 'bg-emerald-100 text-emerald-700' : payment.status === 'Atrasado' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{payment.status}</span></td>
                        <td className="px-4 py-3">
                          {payment.status !== 'Pago' && (
                            <div className="flex gap-2">
                              <button onClick={() => sendWhatsAppCharge(client.id, payment.id)} className="text-emerald-600"><Send size={16} /></button>
                              <button onClick={() => markAsPaid(payment.id)} className="text-blue-600"><Check size={16} /></button>
                            </div>
                          )}
                        </td>
                      </tr>
